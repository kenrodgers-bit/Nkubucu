...
export function AlbumCard({ album }: { album: Album }) {
  function recordCheckAllClick() {
    const url = `/api/albums/${album.slug}/check-all`;
  ...
  return (
    <article className="group overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-soft">
      <Link
        href={`/album/${album.slug}`}
        className="block"
        aria-label={`Open preview page for ${album.title}`}
      >
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <Link href={`/album/${album.slug}`} className="focus-ring rounded-md">
              <h2 className="line-clamp-2 text-base font-semibold leading-6 text-ink transition group-hover:text-tealhub-700">
                {album.title}
              </h2>
            </Link>
        ...
        <a
          ...
          className="focus-ring mt-4 flex h-11 items-center justify-center gap-2 rounded-md bg-green-600 px-4 text-sm font-semibold text-white transition hover:bg-green-700"
        >
          Check All Photos
